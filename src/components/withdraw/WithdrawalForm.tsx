...
import serverCallFuction, { formattedAmountCommas, getCurrencyIcon } from "@/lib/constantFunction";
...
const WithdrawalForm = ({ close }: { close: () => void }) => {
  ...
  useEffect(() => {
    if (amount <= 0) {
      ...
      setError(`Insufficient funds! You only have ₹${currentBalance} available.`);
    } else {
      setError(null);
    }
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="p-4 border border-gray-200 dark:border-gray-800 rounded-xl flex items-center gap-4">
          ...
          <span className="text-sm bg-green-100 text-green-700 px-2 py-1 rounded font-bold">
            {getCurrencyIcon('INR')}{formattedAmountCommas(currentBalance)}
          </span>
        </div>
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              className={`w-full pl-4 pr-12 py-3 bg-gray-50 dark:bg-gray-800 border ${
                error ? 'border-red-500 focus:ring-red-500' : 'border-gray-200 dark:border-gray-700 focus:ring-brand-500'
              } rounded-xl outline-none transition-all`}
              required
              autoComplete="off"
            />
            <span className="absolute right-4 top-3.5 text-gray-400 font-medium">INR</span>
          </div>
        </div>
        ...
        <div className="p-4 border border-gray-200 dark:border-gray-800 rounded-xl flex items-center gap-4">
          ...
          <div className="flex-1">
            <p className="text-sm font-medium text-gray-900 dark:text-white">
              {(user as { bank_name?: string; bankName?: string })?.bank_name || (user as { bank_name?: string; bankName?: string })?.bankName || "No Bank Linked"}
            </p>
            <p className="text-xs text-gray-500">
              Acc: {(user as { account_no?: string; accountNo?: string })?.account_no || (user as { account_no?: string; accountNo?: string })?.accountNo || "0000"}
            </p>
            <p className="text-xs text-gray-500">
              Holder: {(user as { account_holder_name?: string; accountHolderName?: string })?.account_holder_name || (user as { account_holder_name?: string; accountHolderName?: string })?.accountHolderName || "N/A"}
            </p>
            <p className="text-xs text-gray-500">
              Routing/IFSC: {(user as { ifsc_code?: string; ifscCode?: string })?.ifsc_code || (user as { ifsc_code?: string; ifscCode?: string })?.ifscCode || "N/A"}
            </p>
          </div>
